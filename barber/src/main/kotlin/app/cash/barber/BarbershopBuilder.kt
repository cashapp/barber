...
class BarbershopBuilder : Barbershop.Builder {
  @Deprecated(
      "We don't want to compile on install so we can compile with a MustacheFactory according to document output field encoding")
  ...
  private val installedFieldKeyDocumentKParameterMap =
  ...
  override fun installDocument(document: KClass<out Document>) = apply {
    ...
    val documentParametersByFieldKey = document.primaryConstructor!!.asParameterNames()
    documentParametersByFieldKey.forEach { (fieldKey, kParameter) ->
      if (installedFieldKeyDocumentKParameterMap.containsKey(fieldKey)) {
        throw BarberException(errors = listOf("""
          |Attempted to install Document that shadows an already installed Document's fields.
          |Already Installed
          |Document: ${installedFieldKeyDocumentKParameterMap[fieldKey]!!.first}
          |Field: $fieldKey
          |
          |Attempted to Install
          |Document: $document
      ...
      fieldKey?.let { installedFieldKeyDocumentKParameterMap[fieldKey] = Pair(document, kParameter) }
    }
  }
   ...
   * validated elements.
   */
  private fun Table<KClass<out DocumentData>, Locale, DocumentTemplate>.validate(): Table<KClass<out DocumentData>, Locale, CompiledDocumentTemplate> {
    ...
    cellSet().forEach { cell ->
      ...
      val compiledDocumentTemplate =
          documentTemplate.compile(mustacheFactoryProvider, installedFieldKeyDocumentKParameterMap)
    ...
    installedCompiledDocumentTemplates.rowMap().forEach { (documentDataClass, documentTemplates) ->
